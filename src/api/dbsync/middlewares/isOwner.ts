    ...
    switch (method) {
      case 'GET': {
        if (!entryId) {
          ...
          const entry = await strapi.documents('api::dbsync.dbsync').findOne({
            documentId: entryId,
            populate: ['users_permissions_user'],
          }) as DbsyncWithUser | null;

          if (!entry) {
            return ctx.notFound("dbsync not found.");
          }
      ...
      case 'POST': {
        // Enforce 1:1 (one dbsync per user)
        const existing = await strapi.documents('api::dbsync.dbsync').findMany({
        ...
        if (existing && existing.length > 0) {
          return ctx.badRequest("You already have a dbsync. Please update it instead.");
        }
      ...
      case 'DELETE': {
        ...
        // Verify ownership
        const entryToModify = await strapi.documents('api::dbsync.dbsync').findOne({
          documentId: entryId,
          populate: ['users_permissions_user'],
        }) as DbsyncWithUser | null;
